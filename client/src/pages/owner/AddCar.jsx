    ...
    <div className='px-4 py-10 md:px-10 flex-1'>
      ...
      <form onSubmit={onSubmitHandler} className='flex flex-col gap-5 text-gray-500 text-sm mt-6 max-w-xl'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
          ...
          <div className='flex flex-col w-full'>
            ...
            <select onChange={e => setCar({ ...car, category: e.target.value })} value={car.category} className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none'>
              ...
              <option value="sedan">Sedan</option>category
        ...
        <div className='flex flex-col w-full'>
          <div className='flex flex-col w-full'>
            ...
            <select onChange={e => setCar({ ...car, location: e.target.value })} value={car.location} className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none'>
              <option value="Gas">Select a Location</option>
        ...
        <div className='flex flex-col w-full'>
          ...
          <textarea rows={5} placeholder='e.g. A luxurious SUV with a spacious interior and a powerfull engine. ' required className='px-3 py-2 mt-1 border border-borderColor rounded-md resize-none outline-none' value={car.description} onChange={e => setCar({ ...car, description: e.target.value })}></textarea>
        </div>

        <button className='flex items-center gap-2 px-4 py-2.5 mt-4 bg-primary text-white rounded-md font-medium w-max cursor-pointer'><img src={assets.tick_icon} alt="" />{isLoading ? 'Listing...' : 'List your Cars'}</button>
      </form>
    </div>
  )
}
